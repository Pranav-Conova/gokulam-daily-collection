...
export default function App() {
  ...
  // Handle data extracted from image
  const handleDataExtracted = (extractedData) => {
    const branchIndex = rows.findIndex(row => row.branch === extractedData.branch);
    if (branchIndex !== -1) {
      const newRows = [...rows];
      newRows[branchIndex] = {
        ...newRows[branchIndex],
        totalRB: extractedData.totalRB || newRows[branchIndex].totalRB,
        rbColln: extractedData.rbColln || newRows[branchIndex].rbColln,
        totalARR: extractedData.totalARR || newRows[branchIndex].totalARR,
        arrColln: extractedData.arrColln || newRows[branchIndex].arrColln,
        totalColln: extractedData.totalColln || newRows[branchIndex].totalColln,
        bill: extractedData.bill || newRows[branchIndex].bill,
      };
      setRows(newRows);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-50 p-3 sm:p-6 text-gray-900">
      ...
      {/* Professional Download and Share Buttons */}
      <div className="flex justify-center gap-4 mt-6">
        <button
          onClick={downloadTableImage}
          className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-purple-600 to-purple-700 text-white rounded-2xl hover:from-purple-700 hover:to-purple-800 font-semibold transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
        >
        ...
        <button
          ...
          className="inline-flex items-center px-8 py-4 bg-gradient-to-r from-green-500 to-green-600 text-white rounded-2xl hover:from-green-600 hover:to-green-700 font-semibold transition-all duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
        >
